...
const SHIFT_SHORT: Record<string, string> = {
  "24H": "24H", DAY12: "DAY", NIGHT12: "NGT",
}

const SHIFT_LABEL: Record<string, string> = {
  "24H": "24H (9AM–9AM)", DAY12: "Day 12h (9AM–9PM)", NIGHT12: "Night 12h (9PM–9AM)",
}
...
export default function ScheduleGrid({
  ...
  physicians.forEach((p) => { physColorMap[p.id] = p.color ?? "#6366f1" })
  ...
  for (const dateStr of Object.keys(byDate)) {
    byDate[dateStr].sort(
      (a, b) => (ORDER[a.shiftType as keyof typeof ORDER] ?? 0) - (ORDER[b.shiftType as keyof typeof ORDER] ?? 0)
    )
  }
  ...
  for (const p of physicians) shiftCounts[p.id] = { "24H": 0, DAY12: 0, NIGHT12: 0, total: 0 }
  for (const a of assignments) {
  ...
  function AssignmentDropdown({ a }: { a: Assignment }) {
    const doc       = a.user
    const physColor = doc ? (physColorMap[doc.id] ?? "#6366f1") : "#94a3b8"
    return (
      <div
        className="absolute top-full left-0 mt-1 z-50 bg-white rounded-xl shadow-xl ring-1 ring-slate-200 overflow-hidden"
        style={{ minWidth: "220px", maxWidth: "90vw" }}
      >
        <div className="px-3 py-2 bg-slate-50 border-b border-slate-200">
          <p className="text-xs font-semibold text-slate-700">
            {format(new Date(a.date + "T12:00:00"), "EEE, MMM d")} – {a.shiftType}
          </p>
          {a.isConflict && <p className="text-xs text-red-600 mt-0.5">{a.conflictNote}</p>}
        </div>
  ...
  function ShiftBlock({ a, fullWidth }: { a: Assignment; fullWidth?: boolean }) {
    ...
    return (
      <div className="relative">
        ...
        {isOpen && <AssignmentDropdown a={a} />}
      </div>
    )
  }

  const agendaDays = Object.entries(byDate)
    .filter(([, dayAssigns]) => dayAssigns.length > 0)
    .sort(([a], [b]) => a.localeCompare(b))

  return (
    <div className="space-y-4">
      <p className="text-xs text-slate-500">Click a shift block to reassign. 🔒 = manually locked.</p>
      ...
      {isMobile && (
        <div className="space-y-2">
          {agendaDays.length === 0 ? (
            <p className="text-sm text-slate-400 py-8 text-center">No shifts scheduled this month</p>
          ) : agendaDays.map(([dateStr, dayAssigns]) => {
            const isToday = dateStr === todayStr
            return (
              <div
                key={dateStr}
                className={`rounded-xl border p-3 ${isToday ? "border-blue-300 bg-blue-50" : "border-slate-200 bg-white"}`}
              >
                <p className={`text-xs font-semibold mb-2 flex items-center gap-2 ${isToday ? "text-blue-700" : "text-slate-600"}`}>
                ...
                <div className="space-y-1">
                  {dayAssigns.map((a) => <ShiftBlock key={a.id} a={a} fullWidth />)}
                  {onAddSlot && !dayAssigns.some((a) => a.shiftType === "24H") && (
                    <div className="flex gap-2 mt-1">
                      {!dayAssigns.some((a) => a.shiftType === "DAY12") && (
                        <button
                          type="button"
                          onClick={() => handleAddSlot(dateStr, "DAY12")}
                          disabled={addingSlot === `${dateStr}::DAY12`}
                          className="flex-1 text-xs text-amber-600 border border-dashed border-amber-300 rounded px-2 py-1 hover:bg-amber-50 transition"
                        >
                          {addingSlot === `${dateStr}::DAY12` ? "…" : "+ DAY"}
                        </button>
                      )}
                      {!dayAssigns.some((a) => a.shiftType === "NIGHT12") && (
                        <button
                          type="button"
                          onClick={() => handleAddSlot(dateStr, "NIGHT12")}
                          disabled={addingSlot === `${dateStr}::NIGHT12`}
                          className="flex-1 text-xs text-indigo-600 border border-dashed border-indigo-300 rounded px-2 py-1 hover:bg-indigo-50 transition"
                        >
          ...
          {cells.map((cell, idx) => {
            ...
            return (
              ...
              >
                ...
                {dayAssigns.map((a) => <ShiftBlock key={a.id} a={a} />)}
                {onAddSlot && !dayAssigns.some((a) => a.shiftType === "24H") && (
                  <>
                    {!dayAssigns.some((a) => a.shiftType === "DAY12") && (
                      <button
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-1">
              {legendPhysicians.map((p) => (
                <span key={p.id} className="flex items-center gap-1.5">
                  <span className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: physColorMap[p.id] ?? "#6366f1" }} />
          ...
          <div className="flex flex-wrap gap-3">
            {[
              ...
              <span key={s.key} className="flex items-center gap-1 text-slate-600">
                <span className="font-mono text-[10px] bg-slate-100 rounded px-1 py-0.5">{SHIFT_SHORT[s.key]}</span>
                {s.label}
              </span>
            ))}
          </div>
      ...
      {physicians.length > 0 && (
        <div className="mt-6">
          <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">Shift Totals</h3>
          <div className="overflow-x-auto rounded-xl ring-1 ring-slate-200">
            <table className="w-full text-xs min-w-[360px]">
              ...
              <tbody className="divide-y divide-slate-100">
                {[...physicians]
                  .sort((a, b) => { if (a.isPRN !== b.isPRN) return a.isPRN ? 1 : -1; return a.name.localeCompare(b.name) })
                  .map((p) => {
                    ...
                    return (
                      <tr key={p.id} className={`transition ${c.total === 0 ? "bg-red-50" : "bg-white hover:bg-slate-50"}`}>
                        <td className="px-3 py-2 flex items-center gap-2">
                          <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: physColorMap[p.id] ?? "#6366f1" }} />
